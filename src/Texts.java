...
public class Texts {
    ...
    private String getSentence(String[] sentences, int sentenceIndex) {
        return sentences[sentenceIndex];
    }

    public String getTwoSentences() {
        StringBuilder result = new StringBuilder();
        String[] sentences = getRandomText().split("[.]");
        int sentenceIndex = random.nextInt(sentences.length - 1);
        for (int i = 0; i < 2; i++) {
            result.append(getSentence(sentences, sentenceIndex)).append(".");
            sentenceIndex++;
        }
